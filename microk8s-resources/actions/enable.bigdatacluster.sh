...
echo "Installing azdata cli"
echo "----------------------------------------"
pip3 install -r https://aka.ms/azdata --user --no-warn-script-location
azdata_path="${HOME}/.local/bin/azdata"
echo "----------------------------------------"
echo "azdata has been successfully installed in ${azdata_path}"
...
# azdata is used to deploy a customized config of Big Data Clusters onto microk8s.
# The config files are located in ${SNAP}/actions/bdc
echo "----------------------------------------"
echo "Deploying BDC with azdata"
echo "----------------------------------------"
${azdata_path} bdc create --config-profile ${config_profile_path} --accept-eula='yes'
